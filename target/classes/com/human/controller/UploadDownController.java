...
//               userService.saveImg("/upload/" + fileName, "HQ");
               //将文件在服务器的存储路径返回
               return JsonMsg.success().addInfo("success", "http://120.79.15.183:8080/travel/upload/" + fileName);
             //  return JsonMsg.success().addInfo("success", "http://localhost:8082/upload/" + fileName);
           } catch (IOException e) {
    ...
    public JsonMsg uploadResume(@RequestParam("UserResume") MultipartFile UserResume, HttpServletRequest request) {
        try {
            ...
            try {
                ...
                //将文件在服务器的存储路径返回
                return JsonMsg.success().addInfo("success", "http://120.79.15.183:8080/travel/upload/" + fileName);
               // return JsonMsg.success().addInfo("success", "http://localhost:8082/myssmp_war_exploded/myssmp/upload/" + fileName);
            } catch (IOException e) {
                System.out.println("上传失败");
                e.printStackTrace();
